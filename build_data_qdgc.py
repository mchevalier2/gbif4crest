""" Processes all the geophysical datasets into one big data_qdgc table """
...
def rebin_mean(a, shape):
    """Upscale raster a to shape using a mean of values"""
    sh = shape[0], a.shape[0] // shape[0], shape[1], a.shape[1] // shape[1]
    return a.reshape(sh).mean(-1).mean(1)


def rebin_max(a, shape):
    """Upscale raster a to shape using a max of values"""
    sh = shape[0], a.shape[0] // shape[0], shape[1], a.shape[1] // shape[1]
    return a.reshape(sh).max(-1).max(1)


def rebin_mode(a, shape):
    """Upscale raster a to shape using the most abundant value"""
    sh = shape[0], a.shape[0] // shape[0], shape[1], a.shape[1] // shape[1]
    a = a.reshape(sh)
...
def most_common(lst):
    """Estimate most abundant value from list"""
    return max(set(lst), key=lst.count)


def get_neighbours(locid):
    """Get the 9 neighbouring cells of loc"""
    return [
        x
        for x in [
            locid + 1,
...
LATS = pd.DataFrame((np.arange(2160) / 12 - 90 + 1.0 / 24))
LONGS = pd.DataFrame((np.arange(4320) / 12 - 180 + 1.0 / 24))

data_qdgc = LONGS.merge(LATS, how="cross")
data_qdgc.columns = ["longitude", "latitude"]
data_qdgc["locID"] = data_qdgc.apply(
    lambda x: f_locid([x.longitude, x.latitude]), axis=1
)
print(data_qdgc)


if "ETOPO1_Ice_g_geotiff.zip" in os.listdir(TEMP_FOLDER):
    print(" * Adding Elevation data\n")
    data_qdgc = (
        data_qdgc[["locID", "longitude", "latitude"]].sort_values("locID").reset_index()
    )
    os.system(
        f"unzip -o {TEMP_FOLDER}ETOPO1_Ice_g_geotiff -d {TEMP_FOLDER}ETOPO1_Ice_g_geotiff"
    )
    img = gdal_array.LoadFile(
        f"{TEMP_FOLDER}ETOPO1_Ice_g_geotiff/ETOPO1_Ice_g_geotiff.tif"
    )
    lists = []
    for j in range(4320):
        for i in range(2160):
    ...
    df = (
        pd.DataFrame(lists, columns=["locID", "elevation", "elev_min", "elev_max"])
        .sort_values("locID")
        .reset_index()
    )
    ...
    print(
        "MISSING DATASET\n\t> Download ETOPO data: https://www.ngdc.noaa.gov/mgg/global/relief/ETOPO1/data/ice_surface/grid_registered/georeferenced_tiff/ETOPO1_Ice_g_geotiff.zip"
    )

print(os.listdir(TEMP_FOLDER))
if "wc2.1_30s_bio.zip" in os.listdir(TEMP_FOLDER) and "Global-AI_ET0_annual_v3.zip" in os.listdir(TEMP_FOLDER):
    ...
    for biovar in range(1, 20):
        print("biovar", biovar)
        img = gdal_array.LoadFile(
            f"{TEMP_FOLDER}wc2.1_30s_bio/wc2.1_30s_bio_{biovar}.tif"
        )
    ...
    #
    # These two blocks should remain together. I am using the WC mask for AI
    ...
    img_array_masked = np.ma.MaskedArray(img_array, mask=mask_wc)
    ds = rebin_mean(img_array_masked, (2160, 4320)).filled(np.nan)
    data_qdgc["ai"] = ds.flatten(order="F") / 10000.0
    os.system(f"rm -Rf {TEMP_FOLDER}Global-AI_ET0_v3_annual")
else:
    print(
        "MISSING DATASET\n\t> Download worldclim data: https://geodata.ucdavis.edu/climate/worldclim/2_1/base/wc2.1_30s_bio.zip"
    )
    print("\t> Download AI data: https://figshare.com/ndownloader/files/34377245")
...
if True:
    ...
    ECOREGIONS = {}
    eco = pd.read_csv(
        f"{TEMP_FOLDER}ecoregions.csv", dtype={"BIOME": "int", "ECO_ID": "int"}
    )
    for i, row in eco.iterrows():
        ECOREGIONS[row["ECO_ID"]] = row["ECO_NAME"]
    #
    biogeography = pd.read_csv(f"{DATABASE_FOLDER}biogeography.csv")[
        ["ecoregion", "ecoID"]
    ]
    ecoIDs = {}
    ...
    #
    df = (
        pd.DataFrame(ll, columns=["locID", "terr_ecoID"])
        .sort_values("locID")
        .reset_index()
    )
...
if True:
    ...
    countries = pd.read_csv(
        f"{DATABASE_FOLDER}geopolitical_units.csv", encoding="utf-8-sig"
    )[["name", "geopoID", "continent"]]
    countries = duckdb.query("SELECT * FROM countries WHERE continent IS NOT NULL").df()
    countryIDs = {}
    for i, row in countries.iterrows():
        countryIDs[row["name"]] = row["geopoID"]
    #
    countryIDs["S. Geo. and the Is."] = countryIDs[
        "South Georgia and the South Sandwich Islands"
    ]
    countryIDs["Heard I. and McDonald Is."] = countryIDs[
    ...
    countryIDs["U.S. Minor Outlying Is."] = countryIDs[
    ...
    for j in range(4320):
        for i in range(2160):
            if int(ds[i, j]) in COUNTRIES:
                try:
                    ll.append(
                        [j + (2160 - i - 1) * 4320, countryIDs[COUNTRIES[ds[i, j]]]]
                    )
                except KeyError:
                    ll.append([j + (2160 - i - 1) * 4320, np.nan])
    ...
    #
    df = (
        pd.DataFrame(ll, columns=["locID", "countryID"])
        .sort_values("locID")
        .reset_index()
    )
...
if True:
    ...
    locids = list(duckdb.execute(
            "SELECT distinct locid FROM data_qdgc WHERE bio1 IS NOT NULL AND countryID IS NULL"
        ).df()["locID"]
    )
    cnt = run = 0
    while (len(locids) > 0 and len(locids) != cnt) and (run <= 40):
        ...
        res = []
        print(f"Run: {run + 1}. Cells to classify: {len(locids)}")
        for loc in locids:
            tmp = data_qdgc.loc[get_neighbours(loc), ["countryID"]].dropna()
            if len(tmp) > 0:
                res.append([loc, most_common(tmp["countryID"].to_list())])
            else:
                cnt += 1
        run += 1
        if len(res) > 0:
    ...
    print(f"Run: {run}. Cells to classify: {len(locids)}")
    #
    #
    ...
    print(f"Run: {run + 1}. Remaining cells to classify: {len(locids)}")
    data_qdgc.set_index("index", inplace=True)
    data_qdgc.sort_index(inplace=True)
...
if True:
    ...
    locids = list(
        duckdb.execute(
            "SELECT distinct locid FROM data_qdgc WHERE bio1 IS NOT NULL AND terr_ecoID IS NULL"
        ).df()["locID"]
    )
    cnt = run = 0
    while (len(locids) > 0 and len(locids) != cnt) and (run <= 40):
...
if "icec.mon.ltm.1991-2020.nc" in os.listdir(TEMP_FOLDER):
        ...
        for j in range(4320):
            for i in range(2160):
                if not mask[0, i // 3, j // 3]:
                    ...
                    ll.append([j + (2160 - i - 1) * 4320, np.nan])
        df = (
            pd.DataFrame(ll, columns=["locID", season_names[season]])
            .sort_values("locID")
            .reset_index()
        )
    ...
    print(
        "MISSING DATASET\n\t> Download sea ice data: https://www.psl.noaa.gov/data/gridded/data.noaa.oisst.v2.highres.html"
    )
...
if True:
    ...
    for var in ["sst", "sss"]:
        season_names = {
            "sss": ["sss_ann", "sss_jfm", "sss_amj", "sss_jas", "sss_ond"],
            "sst": ["sst_ann", "sst_jfm", "sst_amj", "sst_jas", "sst_ond"],
        }
        for season, file_name in enumerate(
            sorted(glob.glob(f"{TEMP_FOLDER}woa18_decav_{var[2]}*"))
        ):
            ...
            df = (
                pd.DataFrame(ll, columns=["locID", season_names[var][season]])
                .sort_values("locID")
                .reset_index()
            )
    ...
    print(
        "MISSING DATASET\n\t> Download sea ice data: https://www.ncei.noaa.gov/products/world-ocean-atlas"
    )
...
if True:
    ...
    for var, file_name in enumerate(sorted(glob.glob(f"{TEMP_FOLDER}woa18_all*"))):
        print(var_names[var])
        ds = nc.Dataset(file_name)
        ...
        df = (
            pd.DataFrame(ll, columns=["locID", var_names[var]])
            .sort_values("locID")
            .reset_index()
        )
...
if True:
    ...
    while cnt > 0:
        locIDs = (
            duckdb.query(
                "SELECT distinct locid FROM data_qdgc "+
                "WHERE diss_oxy IS NOT NULL AND sss_ann IS NULL"
            )
            .df()["locID"]
            .to_list()
        )
...
if True:
    ...
    while cnt > 0 and run < 11:
        locIDs = (
            duckdb.query(
                "SELECT distinct locid FROM data_qdgc "+
                "WHERE diss_oxy IS NULL AND sss_ann IS NOT NULL"
            )
            .df()["locID"]
            .to_list()
        )
...
if False:
    ...
    while cnt > 0:
        locIDs = (
            duckdb.query(
                "SELECT distinct locid FROM data_qdgc "+
                "WHERE icec_ann IS NULL AND sss_ann IS NOT NULL "+
                "      AND (latitude >=60 OR latitude <= -60)"
            )
            .df()["locID"]
            .to_list()
        )
        ...
        for i, loc in enumerate(locIDs):
            values = data_qdgc.loc[
                ...
                ["icec_ann", "icec_jfm", "icec_amj", "icec_jas", "icec_ond"],
            ].dropna()
            if values.shape[0] > 0:
                res.append([loc] + values.mean(axis=0).to_list())
            else:
...
if True:
    ...
    #
    df = (
        pd.DataFrame(ll, columns=["locID", "oceanID"])
        .sort_values("locID")
        .reset_index()
    )
...
if True:
    ...
    #
    df = (
        pd.DataFrame(ll, columns=["locID", "mari_ecoID"])
        .sort_values("locID")
        .reset_index()
    )
...
if True:
    ...
    locids = list(
        duckdb.execute(
            "SELECT distinct locid FROM data_qdgc "+
            "WHERE sst_ann IS NOT NULL AND mari_ecoID IS NULL"
        ).df()["locID"]
    )
    cnt = run = 0
    while (len(locids) > 0 and len(locids) != cnt) and (run <= 40):
    ...
    if len(locids) > 0:
        print(
            " * Fixing missing mari_ecoID relative to World Ocean Atlas with broader neighbours"
        )
        locids = list(
            duckdb.execute(
                "SELECT distinct locid FROM data_qdgc "+
...
if True:
    ...
    locids = list(
        duckdb.execute(
            "SELECT distinct locid FROM data_qdgc "+
            "WHERE sst_ann IS NOT NULL AND oceanID IS NULL"
        ).df()["locID"]
    )
    cnt = run = 0
    while (len(locids) > 0 and len(locids) != cnt) and (run <= 40):
    ...
    if len(locids) > 0:
        print(
            " * Fixing missing oceanID relative to World Ocean Atlas with broader neighbours"
        )
        locids = list(
            duckdb.execute(
                "SELECT distinct locid FROM data_qdgc "+
...
""" Accepted pylint errors *************
    989:0: C0301: Line too long (102/100) (line-too-long)
    82:3: W0125: Using a conditional statement with a constant value (using-constant-test)
    177:12: C0103: Constant name "biome" doesn't conform to UPPER_CASE naming style (invalid-name)
    178:12: C0103: Constant name "ecoregion" doesn't conform to UPPER_CASE naming style (invalid-name)
    321:4: C0103: Constant name "cnt" doesn't conform to UPPER_CASE naming style (invalid-name)
    321:10: C0103: Constant name "run" doesn't conform to UPPER_CASE naming style (invalid-name)
    478:9: E1101: Module 'netCDF4' has no 'Dataset' member (no-member)
    986:0: W0105: String statement has no effect (pointless-string-statement)
"""
