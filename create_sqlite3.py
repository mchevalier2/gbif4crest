...
data_types = {'geopoid':'INTEGER', 'continent':'VARCHAR(13)', 'basin':'VARCHAR(13)', 'name':'VARCHAR(59)', 'countrycode':'VARCHAR(3)'}
geopolitical_units=geopolitical_units.rename(columns = {'geopoID':'geopoid'})
geopolitical_units.to_sql('geopolitical_units', conn, index=False, index_label='geopoid', dtype=data_types, if_exists='replace')


biogeography = pd.read_csv(DATABASE_FOLDER + 'biogeography.csv')
biogeography=biogeography.rename(columns = {'ecoID':'ecoid'})
biogeography.to_sql('biogeography', conn, index=False, index_label='ecoid', if_exists='replace')


typeofobservations = pd.DataFrame({
    'type_of_obs':[1,2,3,4,5,6,7,8],
    'name':[x.lower() for x in ['HUMAN_OBSERVATION', 'OBSERVATION', 'LIVING_SPECIMEN', 'FOSSIL_SPECIMEN', 'MATERIAL_SAMPLE', 'MACHINE_OBSERVATION', 'LITERATURE', 'UNKNOWN']]
})
typeofobservations.to_sql('typeofobservations', conn, index=False, index_label='type_of_obs', if_exists='replace')


taxalist = pd.read_csv(DATABASE_FOLDER + 'taxalist.csv')
taxalist=taxalist.rename(columns = {'taxonID':'taxonid'})
taxalist[['taxonid', 'kingdom', 'phylum', 'class_name', 'order_name', 'family', 'genus', 'species', 'gbifID', 'nb_occ', 'nb_occ_qdgc']].to_sql('taxa', conn, index=False, index_label='taxonid', if_exists='replace')


data_qdgc = pd.read_csv(DATABASE_FOLDER + 'data_qdgc.csv', dtype={'mari_ecoID':'Int64', 'oceanID':'Int64', 'continentID':'Int64', 'terr_ecoID':'Int64'})
data_qdgc=data_qdgc.rename(columns = {'locID':'locid', 'oceanID':'oceanid', 'continentID':'continentid', 'terr_ecoID':'terr_ecoid', 'mari_ecoID':'mari_ecoid', 'countryID':'countryid'})

data_qdgc.drop("index", axis=1, inplace=True)
#data_qdgc.columns = ['locid', 'longitude', 'latitude', 'mari_ecoid', 'oceanid']
##> Test for oceanid
test = data_qdgc[data_qdgc['oceanid'].notna()].merge(geopolitical_units, how='inner', left_on='oceanid', right_on='geopoid')
assert test.shape[0] == data_qdgc[data_qdgc['oceanid'].notna()].shape[0]
##> Test for continentid
test = data_qdgc[data_qdgc['countryid'].notna()].merge(geopolitical_units, how='inner', left_on='countryid', right_on='geopoid')
assert test.shape[0] == data_qdgc[data_qdgc['countryid'].notna()].shape[0]
##> Test for terr_ecoid
test = data_qdgc[data_qdgc['terr_ecoid'].notna()].merge(biogeography, how='inner', left_on='terr_ecoid', right_on='ecoid')
assert test.shape[0] == data_qdgc[data_qdgc['terr_ecoid'].notna()].shape[0]
##> Test for mari_ecoid
test = data_qdgc[data_qdgc['mari_ecoid'].notna()].merge(biogeography, how='inner', left_on='mari_ecoid', right_on='ecoid')
assert test.shape[0] == data_qdgc[data_qdgc['mari_ecoid'].notna()].shape[0]
data_qdgc.to_sql('data_qdgc', conn, index=False, index_label='taxonid', if_exists='replace')



distrib_qdgc = pd.read_csv(DATABASE_FOLDER + 'distrib_qdgc.csv', dtype = {'first_occ':'Int64', 'last_occ':'Int64'})
...
assert test.shape[0] == distrib_qdgc.shape[0]
distrib_qdgc.to_sql('distrib_qdgc', conn, index=False, index_label='taxonid', if_exists='replace')


params = pd.DataFrame({
    'version':'gbif4crest_03',
    'resol':[1/12]
})
